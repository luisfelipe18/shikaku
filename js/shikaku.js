...
export function initShikaku(root) {
  ...
  function showBest() {
    const v = loadBest()[game.difficulty];
  ...
  function layout() {
    ...
    cellSize = Math.max(20, Math.min(64, Math.floor(available / game.cols)));
  ...
  function render() {
    ...
    for (const rect of game.rects) {
      const style = theme[game.classify(rect)] || theme.open;
      const x = rect.c * cellSize + pad;
      const y = rect.r * cellSize + pad;
      roundRect(x, y, rect.w * cellSize - pad * 2, rect.h * cellSize - pad * 2, 8);
    ...
    if (drag) {
      ...
      roundRect(left * cellSize + pad, top * cellSize + pad, (right - left + 1) * cellSize - pad * 2, (bottom - top + 1) * cellSize - pad * 2, 8);
  ...
  function updateStatus() {
    ...
    progressEl.textContent = `${game.filledCells()}/${total} cells | ${game.validCount()}/${game.clues.length} rectangles`;
    statusEl.textContent = game.solved ? "Solved" : "In progress";
    statusEl.dataset.state = game.solved ? "solved" : "playing";
  }
  ...
  function newGame(difficulty) {
    game = new Game(generatePuzzle(difficulty));
  ...
  function bindControls() {
    ...
    q("undo").addEventListener("click", () => { if (game.undo()) afterMove(); });
    q("clear").addEventListener("click", () => { game.clear(); afterMove(); });
  ...
  return {
    onShow() {
      layout();
      if (game && !game.solved && timerStart === null) startTimer();
    },
    onHide() {
      pause();
      persist();
